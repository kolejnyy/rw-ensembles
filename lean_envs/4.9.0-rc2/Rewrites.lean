...
import invpro.lean.ExprSize

namespace invpro.lean.Rewrites
...
def rwTacticString (steps : List (Expr × Bool)) (locName? : Option Name) : String :=
  let item (e : Expr) (symm : Bool) : String :=
    match getAppFn e with
    | .const n _ => (if symm then "← " else "") ++ toString n
    | _ => "?"
  let parts := steps.map (fun (e, s) => item e s)
  let rwCore := "rw [" ++ (", ".intercalate parts) ++ "]"
  match locName? with
  | some n => rwCore ++ " at " ++ toString n
  | none => rwCore
...
syntax (name := rwcnc) "rwcnc" (num)? (num)? (ppSpace rwcncOnlySimplify)? (ppSpace location)? : tactic

@[tactic rwcnc] def elabRwcnc : Tactic := fun stx => do
  let `(tactic| rwcnc $[$n:num]? $[$d:num]? $[$onlySimplify:rwcncOnlySimplify]? $[$loc:location]?) := stx
    | throwUnsupportedSyntax
  let tk := stx
  let maxPerStep := n.map (·.getNat) |>.getD 20
  let depth := d.map (·.getNat) |>.getD 1
  let filterOnlySimplify := !onlySimplify.isNone
  let moduleRef ← createModuleTreeRef
  let forbidden : NameSet := ∅
  reportOutOfHeartbeats `findRewrites tk
  let goal ← getMainGoal

  let locOpt := if stx[4].isNone then none else some (stx[4][0])
      ...
      let rec goHyp (mctx : MetavarContext) (stepsLeft : Nat) (goal : MVarId) (target : Expr)
        ...
        for r in results do
          ...
          let (typeStr, newComplexity, newDepth) ← withMCtx r.mctx do
            ...
            pure (fmt.pretty, invpro.lean.ExprSize.formatNumComponents fmt, invpro.lean.ExprSize.formatMaxDepth fmt)
