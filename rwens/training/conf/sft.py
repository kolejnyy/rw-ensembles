...
from rwens.training.conf.utils import (
    deep_merge,
    get_nested_value,
    load_yaml_config,
    resolve_path,
...
def load_sft_config(
    ...
    if base_config_path is None:
        ...
        if default_base.exists():
            ...
            # Path(__file__) is rwens/training/conf/sft.py
            # parents[0] = conf/, parents[1] = training/, parents[2] = rwens/ (package), parents[3] = rwens/ (project root)
            project_root = Path(__file__).resolve().parents[3]  # Go up from rwens/training/conf/sft.py to project root
