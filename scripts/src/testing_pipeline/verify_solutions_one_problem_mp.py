#!/usr/bin/env python3
"""Verify one problem's attempts under a solutions directory using multiprocessing."""
...
_WORKER_INITIAL_IMPORTS: Optional[str] = None
...
def _is_lsp_crash_like_error(err: Optional[str]) -> bool:
    if not err:
        return False
    s = str(err).lower()
    return "-32901" in s or "-32902" in s or "server process" in s and "crashed" in s or "lsp error" in s or "broken pipe" in s or "connection reset" in s


def _init_worker(project_root: str, initial_imports: str, timeout_seconds: float) -> None:
    global _WORKER_VERIFIER, _WORKER_PROJECT_ROOT, _WORKER_INITIAL_IMPORTS, _WORKER_TIMEOUT_SECONDS
    _WORKER_PROJECT_ROOT = project_root
    _WORKER_INITIAL_IMPORTS = initial_imports
    _WORKER_TIMEOUT_SECONDS = timeout_seconds
    _WORKER_VERIFIER = ProofVerifier(project_root=project_root, initial_imports=initial_imports, timeout_seconds=timeout_seconds)


def _reset_worker_verifier() -> None:
    global _WORKER_VERIFIER
    _WORKER_VERIFIER = ProofVerifier(
        project_root=str(_WORKER_PROJECT_ROOT),
        initial_imports=str(_WORKER_INITIAL_IMPORTS),
        timeout_seconds=float(_WORKER_TIMEOUT_SECONDS),
    )


def _verify_code_worker(payload: tuple[int, str]) -> tuple[int, bool, str]:
    idx, code = payload
    if _WORKER_VERIFIER is None:
        raise RuntimeError("Worker verifier is not initialized")
    try:
        ok, err = _WORKER_VERIFIER.verify(code)
    except Exception as e:
        if _is_lsp_crash_like_error(str(e)):
            _reset_worker_verifier()
        return idx, False, str(e)
    if (not ok) and _is_lsp_crash_like_error(err):
        _reset_worker_verifier()
        ok, err = _WORKER_VERIFIER.verify(code)
    return idx, ok, "" if err is None else str(err)
...
def _problem_dirs_with_attempts(solutions_dir: Path, attempts_file: str) -> list[Path]:
    return [p for p in sorted(solutions_dir.iterdir()) if p.is_dir() and (p / attempts_file).is_file()]
...
def main() -> int:
    ...
    payload = [(i, c) for i, c in enumerate(codes)]
    successes: list[Optional[bool]] = [None] * len(codes)
    failures: list[tuple[int, str]] = []
    if args.num_workers <= 1:
        _init_worker(str(args.project_root), args.preamble, float(args.timeout_seconds))
        iterator = map(_verify_code_worker, payload)
    else:
        ...
        iterator = pool.imap(_verify_code_worker, payload, chunksize=1)
    try:
        for idx, ok, err in iterator:
            successes[idx] = ok
            if not ok:
