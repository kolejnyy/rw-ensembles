...
Can be run as a worker subprocess:
  python -m invpro.dataset.generate_augmented <lean_path> <project_root> <out_dir>
...
from invpro.utils.applier import StateFetchAbort
from invpro.multithreading.utils import run_with_timeout
from invpro.utils.prefix_preprocessing import fix_indentation
from invpro.utils.splitter import TacticSplitter
from invpro.utils.state import extract_state_text
from invpro.dataset.utils import should_discard_file, split_declarations_theorem_proof
...
def process_one_file(
    *,
    ...
    set_log_file(out_dir / "invpro.log")
...
def main() -> int:
    ...
    if len(sys.argv) != 4:
        out = {
            "ok": False,
            "error": "usage: python -m invpro.dataset.generate_augmented <lean_path> <project_root> <out_dir>",
        }
